...
object ImageWriter {
   ...
   */
  def writeToImage[U](fileName: String, rawImage: RawImage[U], keepValue: U => Boolean, colorDecider: ColorStrategy[U])(
    ...
    Future {
      ...
      (0 until rawImage.height)
        .zip(rawImage.content)
        .toList
        .collect {
          case (rowIndex, rowData) =>
            (0 until rawImage.width).zip(rowData).collect {
              case (colIndex, pixel) =>
